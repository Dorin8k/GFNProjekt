...
class Haushaltsverwaltung:
    ...
    def _create_tables(self):
        ...
        self.cursor.execute("INSERT OR IGNORE INTO Eintraege (eintragid,  planid, name, wert, bereich, typ, datum) VALUES (1, 1, 'Gehalt', 2500, 'Arbeit', 'Einkommen', '12.01.2025')")
        self.cursor.execute("INSERT OR IGNORE INTO Eintraege (eintragid, planid, name, wert, bereich, typ, datum) VALUES (2, 1, 'Asiatisch Essen', 30, 'Verpflegung', 'Ausgaben', '12.01.2025')")
        self.cursor.execute("INSERT OR IGNORE INTO Eintraege (eintragid, planid, name, wert, bereich, typ, datum) VALUES (3, 1, 'Europa Park', 250, 'Freizeit', 'Ausgaben', '12.01.2025')")
    ...
    def autoAddEntry(self, oldTime, planid):
        ...
        currentTime = datetime.strftime(currentTime, "%d.%m.%Y")
        currentTime = datetime.strptime(currentTime, "%d.%m.%Y")
        oldTime = datetime.strftime(oldTime, "%d.%m.%Y")
        oldTime = datetime.strptime(oldTime, "%d.%m.%Y")
        self.cursor.execute("""SELECT e.datum, r.von, r.bis, r.intervall, e.wert, e.name, e.typ, e.bereich, r.reihenID FROM Reihe r
        ...
        for value in result:
            ...
            if currentTime > datumVon and oldTime < datumBis:
                ...
                intervall = relativedelta(days=20)
                match value[3]:
                    case "täglich":
                        intervall = relativedelta(days=1)
                    case "wöchentlich":
                        intervall = relativedelta(days=7)
                    case "monatlich":
                ...
                while (i < 5000):
                    if nextDate >= datumVon and nextDate <= datumBis and nextDate <= currentTime and nextDate > oldTime:
                        ...
                        Haushaltsverwaltung.addAutoEintrag(self, planid, value[5], value[4], value[7], value[6], nextEntryDatum, value[8])
    ...
    def addAutoEintrag(self, planid, name, wert, bereich, typ, datum, reihe):
        self.cursor.execute("INSERT INTO Eintraege (planid, name, wert, bereich, typ, datum, reihe) VALUES (?, ?, ?, ?, ?, ?, ?)", (planid, name, wert, bereich, typ, datum, reihe))
        self.conn.commit()

    def addEintrag(self, planid, name, wert, bereich, typ, datum, intervall=None, von=None, bis=None):
        if intervall and von and bis:
            self.cursor.execute("SELECT MAX(eintragID) FROM Eintraege")
            eintragID = self.cursor.fetchone()[0] + 1
            self.addWiederkehrende(eintragID, intervall, von, bis)
            self.cursor.execute("SELECT reihenID FROM Reihe WHERE eintragid = ?", (eintragID, ))
            reihenID = self.cursor.fetchone()[0]
            self.cursor.execute("INSERT INTO Eintraege (planid, name, wert, bereich, typ, datum, reihe) VALUES (?, ?, ?, ?, ?, ?, ?)",
                                (planid, name, wert, bereich, typ, datum, reihenID))
            self.conn.commit()
        else:
            self.cursor.execute("INSERT INTO Eintraege (planid, name, wert, bereich, typ, datum) VALUES (?, ?, ?, ?, ?, ?)",
                            (planid, name, wert, bereich, typ, datum))
            self.conn.commit()
