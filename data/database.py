...
class Haushaltsverwaltung:
    ...
    def _create_tables(self):
        ...
        self.cursor.execute(
            "INSERT OR IGNORE INTO Haushaltsplaene (id, name, lastChecked) VALUES (1, 'Haushaltsplan Beispiel', '2025-02-13')")
        self.cursor.execute(
            "INSERT OR IGNORE INTO Haushaltsplaene (id, name, lastChecked) VALUES (2, 'Haushaltsplan (inaktiv)', '2025-02-13')")
        self.cursor.execute(
            "INSERT OR IGNORE INTO Haushaltsplaene (id, name, lastChecked) VALUES (3, 'Haushaltsplan (inaktiv)', '2025-02-13')")
        ...
        self.cursor.execute(
            "INSERT OR IGNORE INTO Eintraege (eintragid,  planid, name, wert, bereich, typ, datum, reihe) VALUES (1, 1, 'Gehalt', 2500, 'Arbeit', 'Einkommen', '2025-01-12', 1)")
        self.cursor.execute(
            "INSERT OR IGNORE INTO Eintraege (eintragid, planid, name, wert, bereich, typ, datum, reihe) VALUES (2, 1, 'Asiatisch Essen', 30, 'Verpflegung', 'Ausgaben', '2025-01-11', 1)")
        self.cursor.execute(
            "INSERT OR IGNORE INTO Eintraege (eintragid, planid, name, wert, bereich, typ, datum, reihe) VALUES (3, 1, 'Europa Park', 250, 'Freizeit', 'Ausgaben', '2025-01-12', 1)")
    ...
    def addEintrag(self, planid, name, wert, bereich, typ, datum, reihe=None, intervall=None, von=None, bis=None):
        self.cursor.execute(
            "INSERT INTO Eintraege (planid, name, wert, bereich, typ, datum, reihe) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    def addWiederkehrende(self, eintragID, intervall, von, bis):
        ...
        while currentDate <= endDate:
            ...
            if currentDate.month == currentMonth and currentDate.year == currentYear:
                ...
                self.cursor.execute(
                    "INSERT INTO Eintraege (planid, name, wert, bereich, typ, datum, reihe) SELECT planid, name, wert, bereich, typ, ?, reihe FROM Eintraege WHERE eintragid = ?",
                    (newDate, eintragID))
    ...
    def fetchAllEintraege(self, planid, sort_by=None, filter_name=None, filter_bereich=None,
        ...
        query = """
        SELECT
        e.name, e.wert, e.bereich, e.typ, e.datum, e.reihe, r.intervall, r.von, r.bis
        FROM
        Eintraege
        e
        LEFT
