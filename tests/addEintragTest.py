...
def test_addEintrag(haushaltsverwaltung):
    ...
    for i in range(5):
        ...
        wert = random.uniform(10, 100)  # Zufälliger Wert zwischen 10 und 100
    ...
    for i in range(5):
        ...
        bereich = random.choice(['Essen', 'Transport', 'Freizeit', 'Sonstiges'])
        typ = random.choice(['Ausgaben', 'Einkommen'])
        von = today.strftime("%Y-%m-%d")  # Startdatum ist heute
        bis = (today + timedelta(days=random.randint(30, 60))).strftime("%Y-%m-%d")  # Enddatum in 30-60 Tagen
